...
export default function Shoppage() {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-10 lg:p-20">
      <div className="relative bg-cover bg-center h-48 sm:h-60 w-full mb-10">
        <img
          src={bannerImage}
          alt="Banner"
          className="w-full h-full object-cover"
        />
        <h1 className="absolute inset-0 flex items-center justify-center text-white text-xl sm:text-3xl font-bold">
          Products
        </h1>
      </div>

      <div className="flex flex-col sm:flex-row justify-between items-center mt-8 mb-10 gap-4">
        <select
          onChange={(e) => setFilter(e.target.value)}
          value={filter}
          className="p-2 border rounded-md hover:bg-black-800 hover:text-white bg-blue-600 text-white w-full sm:w-auto">
      ...
      {filteredProducts.length === 0 ? (
        <p className="text-center font-bold text-2xl sm:text-3xl md:text-5xl mt-10">Product Not Found !!</p>
      ) : (
            ...
            >
              ...
              <Link to={`/product/${product.id}`}>
                <img
                  ...
                  className="mb-3 w-full h-40 object-contain mx-auto"/>
              </Link>
